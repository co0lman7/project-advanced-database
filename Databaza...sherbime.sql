...
CREATE TABLE Reservation (
    ...
    CONSTRAINT PK_Reservation PRIMARY KEY (reservation_id, [date]),
...
) ON ps_ReservationDate([date]);
GO
-- Unique index stored on PRIMARY filegroup (non-aligned) to bypass partition column requirement
CREATE UNIQUE NONCLUSTERED INDEX UX_Reservation_ReservationID
...
CREATE OR ALTER PROCEDURE sp_SafeSearchUsers
...
BEGIN
    ...
    -- Build LIKE parameters safely first
    DECLARE @Name NVARCHAR(100) = NULL;
    DECLARE @Email NVARCHAR(100) = NULL;

    IF @SearchName IS NOT NULL
        SET @Name = N'%' + @SearchName + N'%';

    IF @SearchEmail IS NOT NULL
        SET @Email = N'%' + @SearchEmail + N'%';
...
CREATE OR ALTER TRIGGER trg_PreventDoubleReservation
ON Reservation
...
BEGIN
    ...
    IF EXISTS (
        ...
        FROM inserted i
        JOIN Reservation r
            ON i.professional_id = r.professional_id
        WHERE r.status NOT IN ('cancelled')
          AND i.[date] = r.[date]
          AND i.[time] = r.[time]
          AND (r.reservation_id != i.reservation_id OR i.reservation_id IS NULL)
    )
    BEGIN
    ...
    BEGIN
        INSERT INTO Reservation (user_id, professional_id, service_id, [date], [time], status, created_at)
        SELECT
            user_id,
            professional_id,
            service_id,
    ...
    BEGIN
        ...
        SET
            r.user_id = i.user_id,
            r.professional_id = i.professional_id,
            r.service_id = i.service_id,
            r.[date] = i.[date],
            r.[time] = i.[time],
            r.status = i.status
        FROM Reservation r
...
CREATE OR ALTER TRIGGER trg_UpdateReservationPaymentStatus
ON Payment
AFTER INSERT, UPDATE
AS
BEGIN
...
BEGIN
    ...
    SELECT reservation_id, rating, comment, SYSDATETIME()
    FROM inserted;
END;
GO


CREATE OR ALTER TRIGGER trg_LogReservationDeletion
ON Reservation
AFTER DELETE
AS
BEGIN
    ...
    INSERT INTO ReservationAuditLog (ReservationID, UserID, ProfessionalID, ReservationDate, ReservationTime, Status)
    SELECT reservation_id, user_id, professional_id, [date], [time], status
    FROM deleted;
END;

CREATE OR ALTER TRIGGER trg_PreventCompletedReservationEdit
ON Reservation
AFTER UPDATE
AS
BEGIN
